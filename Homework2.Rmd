...
$$
(X ~|~Y = 0) \sim Unif(-3, 1)~~~ and~~~ (X ~|~Y = 1) \sim Unif(-1, 3) 
$$
and supposing $\mathbb{P}(Y=0)=\mathbb{P}(Y=1)= \frac{1}{2}$ we have that the Bayes classification rule $h_{opt}(x)$ is given by:
...
```{r getting the data, echo=FALSE, include=FALSE}
...
Right after loading the dataset we encode the responses: 

```{r encoding the responses}
y_tr = (y_tr == "book")
y_te = (y_te == "book")
```
...
tolerance = 0.0001 # tolerance for the convergence condition
epochs = 10
...
Starting the raining with the Gradient Descent
...
while (continue) {
  ...
  if ((old.val.loss <= val.loss + tolerance) || (it>epochs)){continue = F}
  it = it + 1
}
